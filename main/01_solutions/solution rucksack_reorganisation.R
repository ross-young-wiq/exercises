...
# coding exercise: rucksack reorganisation
...
#' One team member has the important job of loading all of the rucksacks with supplies for the jungle journey. 
#' Unfortunately, that team member didn't quite follow the packing instructions, and so a few items now need 
#' to be rearranged.
#' 
#' Each rucksack has two large compartments. 
#' All items of a given type are meant to go into exactly one of the two compartments. 
#' The team member that did the packing failed to follow this rule for exactly one item type per rucksack.
#' 
#' Your team have made a list of all of the items currently in each rucksack (your puzzle input), 
...
puzzle_input <- "main/00_data/rucksack_reorganisation.txt"
...
puzzle_input %>% 
  ...
  # check intersection
  .[, V4 := map2_chr(.x = V3, .y = V2, .f = ~ intersect(x = head(.x, .y / 2), y = tail(.x, .y / 2)))] %>% 
  
  # calculate priority
  .[, V5 := map_int(.x = V4, .f = ~ match(.x, c(letters, LETTERS)))] %>% 
