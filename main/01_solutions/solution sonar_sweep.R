...
puzzle_input <- "main/00_data/sonar_sweep.txt"
...
puzzle_input %>% 
  ...
  # take the difference from each element to the next.
  diff() %>% 
...
puzzle_input %>% 
  ...
  # take rolling three-measurement window sum
  slider::slide_dbl(.f = sum, .after = 2, .complete = TRUE) %>% 
